...
var PAYLOAD_PROPERTIES = {
    ...
    'maintain_resolvers': {'type': 'boolean'},
...
var joyent_allowed = {
    ...
    'maintain_resolvers': ['create', 'receive', 'update'],
...
function lookupConflicts(macs, ips, vrids, log, callback) {
    ...
    getZoneRecords(null, {log: log}, function (err, results) {
        ...
        async.eachSeries(vms, function (uuid, cb) {
            vmload.getVmobj(uuid, {log: log}, function (error, obj) {
                var ip;
                var mac;
                var vrid;
...
function createZoneUUID(payload, log, callback)
{
    ...
    if (payload.hasOwnProperty('uuid')) {
        ...
        getZoneRecords(null, {log: log}, function (err, records) {
            if (err) {
                callback(err);
            } else {
        ...
        execFile('/usr/bin/uuid', ['-v', '4'], function (err, stdout, stderr) {
            ...
            getZoneRecords(null, {log: log}, function (e, records) {
                if (e) {
                    callback(e);
                } else {
                    if (records.hasOwnProperty(payload.uuid)) {
...
function ensureFwLogging(action) {
    if (VM.fw_log) {
        return VM.fw_log;
    }

    VM.fw_log = fwlog.create({ 'action': action });
    return VM.fw_log;
}
...
function validateFirewall(payload, callback)
{
    ...
    var log = ensureFwLogging('vm-create');
...
function updateFirewallData(payload, vmobj, callback)
{
    ...
    var firewallOpts = payload.firewall;
    var log = ensureFwLogging('update');
